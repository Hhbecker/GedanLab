...
pdf(file = "RegionalB1.pdf", onefile = TRUE)
ggplot(noOutliersTable, aes(AllSpeciesBiomass, Distichlis.spicata, colour = marshName))+ scale_color_discrete(name = "Marsh Name")  + 
  geom_point(size=3) + ylab("% Distichlis")+xlab("All Species Biomass") +
  ggtitle("Regional Scale All Years") + scale_x_continuous(limits = c(0,1800)) + scale_y_continuous(limits = c(0,500)) + 
...
pdf(file = "RegionalB2.pdf", onefile = TRUE)
ggplot(noOutliersTable, aes(AllSpeciesBiomass, Distichlis.spicata, colour = EOYBYear))+ scale_color_continuous(name = "Year")  + 
  geom_point(size=3) + ylab("% Distichlis")+xlab("All Species Biomass") +
  ggtitle("Regional Scale All Years") + scale_x_continuous(limits = c(0,1800)) + scale_y_continuous(limits = c(0,500)) + 
  stat_smooth(method = lm, se = FALSE, linetype = "dashed", colour="Red", size = 0.6) + 
  geom_smooth(method = 'glm', linetype = "dashed", se = FALSE, colour="Navy", size = 0.5, method.args = list(family = 'poisson'))
...
pdf(file = "PoissonResidualsRegionalB.pdf", onefile = TRUE)
  
  #residual graph code for poisson model 
  Poisson_residual <- glm(Distichlis.spicata ~ AllSpeciesBiomass, family="poisson", data = noOutliersTable)
...
pdf(file = "LinearResidualsRegionalB.pdf", onefile = TRUE)
  
  #residual graph code for linear model
  linModresidual <- lm(Distichlis.spicata ~ AllSpeciesBiomass, data = noOutliersTable )
  ### !!!!!! change this ish around so it matches what you plug it in to !!!
...
#creates an empty list that the for loop will fill
  ...
  Linear_model <- tidy(lm(Distichlis.spicata ~ AllSpeciesBiomass, data = noOutliersTable))
  #View(Linear_model)
  ...
  Linear_model_2 <- glance(lm(Distichlis.spicata ~ AllSpeciesBiomass, data = noOutliersTable))
  #View(Linear_model_2)
  ...
  Poisson_model <- tidy(glm(Distichlis.spicata ~ AllSpeciesBiomass, family="poisson", data = noOutliersTable))
  summary(Poisson_model)
  
  # adjusted r squared is different for poisson, we have to use a specific line of code to get it
  poisson_r <- glm(Distichlis.spicata ~ AllSpeciesBiomass, family="poisson", data = noOutliersTable)
  summary(poisson_r)
...
write_xlsx(model_output, "~/Desktop/distichlis\\RegionalBModels.xlsx")
