...
outlierRemover <- function(x){
  ...
  outliersIn <- filter(transitionZone, marshName == x)
...
pdf(file = "ZoneScale_transition.pdf", onefile = TRUE)

#function for graphing Replicate Scale (manually change marshes )
zoneScaleGrapher <- function(x){
  
  TransitionFrame <- filter(noOutliersTable, marshName == x)
  
  ggplot(TransitionFrame, aes(AllSpeciesBiomass, Distichlis.spicata, colour = EOYBYear))+ scale_color_continuous(breaks = round) +
    geom_point(size=3)+labs(col = "EOYBYear")+ylab('Distichlis g/m2')+xlab('All Species Biomass minus Distichlis g/m2')+
    ggtitle(x, "Zone Scale - Transition Zone")+scale_x_continuous(limits = c(0,2200)) + scale_y_continuous(limits = c(0,600)) + 
...
lapply(marshes, zoneScaleGrapher)
dev.off()
...
for(x in marshes){
  ...
  Linear_model <- tidy(lm(Distichlis.spicata ~ AllSpeciesBiomass, data = TransitionFrame))
  #View(Linear_model)
  ...
  Linear_model_2 <- glance(lm(Distichlis.spicata ~ AllSpeciesBiomass, data = TransitionFrame))
  #View(Linear_model_2)
  ...
  Poisson_model <- tidy(glm(Distichlis.spicata ~ AllSpeciesBiomass, family="poisson", data = TransitionFrame))
  summary(Poisson_model)
  
  # adjusted r squared is different for poisson, we have to use a specific line of code to get it
  poisson_r <- glm(Distichlis.spicata ~ AllSpeciesBiomass, family="poisson", data = TransitionFrame)
  summary(poisson_r)
...
zone_Scale_Transition <- bind_rows(modelOutputList)


#export combined dataframe as an excel to desktop
write_xlsx(zone_Scale_Transition , "~/Desktop/distichlis\\ZoneScale_transition_minusD.xlsx")


#########################################
#residual Grpahs

pdf(file = "PoissonResidualsTransitionZone.pdf", onefile = TRUE)
...
poissonResidualGrapher <- function(x){
  ...
  Poisson_residual <- glm(Distichlis.spicata ~ AllSpeciesBiomass, family="poisson", data = TransitionFrame)
  ### !!!!!! change this ish around so it matches what you plug it in to !!!
  ggplot(Poisson_residual, aes(x = .fitted, y = .resid)) + geom_point()+
    ggtitle(x, "Zone Scale Transition")+ylab('Residuals')+
...
#function for graphing Replicate Scale (manually change marshes )
pdf(file = "LinearResidualsTransitionZone.pdf", onefile = TRUE)
...
linearResidualGrapher <- function(x){
  ...
  linModresidual <- lm(Distichlis.spicata ~ AllSpeciesBiomass, data = TransitionFrame )
  ### !!!!!! change this ish around so it matches what you plug it in to !!!
