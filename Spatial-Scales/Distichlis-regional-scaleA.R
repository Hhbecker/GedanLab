...
years <- c(1999, 2001, 2002, 2004, 2005, 2007:2016)
...
outlierRemover <- function(x){
  ...
  outliersIn <- filter(siteScale, EOYBYear == x)
...
noOutliersList <- lapply(years, outlierRemover)
...
pdf(file = "RegionalA.pdf", onefile = TRUE)
#function for graphing regional style A
regionalAGrapher <- function(x){
  
  regionalAFrame <- filter(noOutliersTable, EOYBYear == x)
  
  ggplot(regionalAFrame, aes(AllSpeciesBiomass, Distichlis.spicata, colour = marshName))+ scale_color_discrete(name = "Marsh Name")  + 
    geom_point(size=3) + ylab("Distichlis Biomass (g/m^2)")+xlab("Other Species Biomass (g/m^2)") +
    ggtitle(x, "Regional Scale") + scale_x_continuous(limits = c(0,1800)) + scale_y_continuous(limits = c(0,500)) + 
    stat_smooth(method = lm, se = FALSE, linetype = "dashed", colour="Red", size = 0.6) + 
    geom_smooth(method = 'glm', linetype = "dashed", se = FALSE, colour="Navy", size = 0.5, method.args = list(family = 'poisson')) +
    scale_colour_manual(values = c("Box Tree" = "darkolivegreen4", 
                                   "Brownsville North Left" = "goldenrod2", "Cushmans Landing" = "coral1", 
                                   "Gator Tract" = "darkslateblue", "Hog Island North" = "dodgerblue", 
                                   "Hog Island South" = "firebrick", "Indiantown" = "burlywood4", "Oyster" = "plum1", 
                                   "Steelmans Landing" = "mediumpurple3", "Upper Phillips" = "aquamarine3" ))
  
}
#apply the function to each item in the list "marshes"
lapply(years, regionalAGrapher)
dev.off()
...
pdf(file = "PoissonResidualsRegionalA.pdf", onefile = TRUE)
...
pdf(file = "LinearResidualsRegionalA.pdf", onefile = TRUE)
...
linearResidualGrapher <- function(x){
  ...
  linModresidual <- lm(Distichlis.spicata ~ AllSpeciesBiomass, data = RegionalAFrame )
  ### !!!!!! change this ish around so it matches what you plug it in to !!!
  ggplot(linModresidual, aes(x = .fitted, y = .resid)) + geom_point()+
    ggtitle(x, "Regional Scale A")+ylab('Residuals')+
    xlab('Fitted Values\nLinear Model(Distichlis ~ All Species Biomass(-Distichlis))')+
    geom_hline(yintercept=0, linetype='dotted')
  
}    
lapply(years, linearResidualGrapher)
dev.off()
...
for(x in years){
  ...
  Linear_model <- tidy(lm(Distichlis.spicata ~ AllSpeciesBiomass, data = regionalAFrame))
  #View(Linear_model)
  ...
  Linear_model_2 <- glance(lm(Distichlis.spicata ~ AllSpeciesBiomass, data = regionalAFrame))
  #View(Linear_model_2)
  ...
  Poisson_model <- tidy(glm(Distichlis.spicata ~ AllSpeciesBiomass, family="poisson", data = regionalAFrame))
  summary(Poisson_model)
  
  # adjusted r squared is different for poisson, we have to use a specific line of code to get it
  poisson_r <- glm(Distichlis.spicata ~ AllSpeciesBiomass, family="poisson", data = regionalAFrame)
  summary(poisson_r)
...
RegionalAModels <- bind_rows(modelOutputList)


#export combined dataframe as an excel to desktop
write_xlsx(RegionalAModels, "~/Desktop/distichlis\\RegionalAModels.xlsx")
