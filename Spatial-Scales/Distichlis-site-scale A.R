...
pdf(file = "PoissonResidualsSiteA.pdf", onefile = TRUE)
...
for(x in marshes){
  ...
  Linear_model <- tidy(lm(Distichlis.spicata ~ AllSpeciesBiomass, data = siteAFrame))
  #View(Linear_model)
  ...
  Linear_model_2 <- glance(lm(Distichlis.spicata ~ AllSpeciesBiomass, data = siteAFrame))
  #View(Linear_model_2)
  ...
  Poisson_model <- tidy(glm(Distichlis.spicata ~ AllSpeciesBiomass, family="poisson", data = siteAFrame))
  summary(Poisson_model)
  
  # adjusted r squared is different for poisson, we have to use a specific line of code to get it
  poisson_r <- glm(Distichlis.spicata ~ AllSpeciesBiomass, family="poisson", data = siteAFrame)
  summary(poisson_r)
